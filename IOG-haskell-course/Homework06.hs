...
factorial n = expression ++ " = " ++ show result
  where
    expression = foldr (\x acc -> show x ++ "*" ++ acc) (show n) [1..n-1]
    result = product [1..n]
...
calculateCost ((beer, quantity):rest) = costOfTheBeer + calculateCost rest
  where 
    theBeer = head $ filter (\(b,_) -> b == beer) bevogBeerPrices
    priceOfTheBeer = snd theBeer
    costOfTheBeer = priceOfTheBeer * quantity