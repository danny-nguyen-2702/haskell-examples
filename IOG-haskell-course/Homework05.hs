...
hasUpperCaseLetter :: String -> Bool
hasUpperCaseLetter = any (`elem` ['a'..'z'])
...
count :: String -> Int
count str = length $ filter (== str) votes
...
checkCarInventory :: String -> Bool
checkCarInventory brand = any (\(b, c) -> b == brand && c > 0) cars